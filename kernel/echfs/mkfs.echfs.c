...
int echfs_mkfs_main(int argc, char **argv) {
    ...
    if (argc < 4) {
          serial_printf("stderr: %s: usage: %s <image> <bytes per block> <reserved blocks factor>\n", argv[0], argv[0]);
          return 1;
    }
    image = fopen(argv[1], "rb");
    if (image == -1) {
         serial_printf("stderr: %s: error: no valid image specified.\n", argv[0]);
    }

    serial_printf("Image size1\n");
    fseek(image, 0L, SEEK_END);
    serial_printf("Image size\n");
    uint64_t imgsize = (uint64_t)ftell(image);
    rewind(image);
    fclose(image);
    image = fopen(argv[1], "r+");

    serial_printf("stderr: %s: info: formatting %lu bytes...\n", argv[0], imgsize);

    uint64_t bytesperblock = atoi(argv[2]);

    if ((bytesperblock <= 0) || (bytesperblock % 512)) {
        serial_printf("stderr: %s: error: block size MUST be a multiple of 512.\n", argv[0]);
        fclose(image);
        return 1;
    }

    if (imgsize % bytesperblock) {
        serial_printf("stderr: %s: error: image is not block-aligned.\n", argv[0]);
        fclose(image);
        return 1;
    }
    ...
    if ((reserved_factor <= 0) || (reserved_factor >= 100)) {
        serial_printf("stderr: %s: error: reserved blocks factor must be between 1%% and 99%%\n", argv[0]);
        fclose(image);
        return 1;
    }
