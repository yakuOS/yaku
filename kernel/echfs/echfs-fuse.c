...
static inline uint16_t rd_word(long loc) {
    ...
    if (ret != 1)
        serial_printf("echfs fuse stderr error reading word!\n");
    return x;
}
...
static inline uint64_t rd_qword(long loc) {
    ...
    if (ret != 1)
        serial_printf("echfs fuse stderr error reading qword!\n");
    return x;
}
...
static struct path_result_table init_table(uint64_t size) {
    serial_printf("init_table\n");
    struct path_result_table table = {0};
    // table.table = 0;
    // table.size = 0;
    // table.num_elements = 0;
    // memset(&table, 0, sizeof(struct path_result_table));
    serial_printf("init_table: size =\n");
    table.table = calloc(sizeof(struct path_result_t*), size);
    serial_printf("init_table: table = %p\n", table.table);
    table.size = size;
    serial_printf("init_table: table->size = %d\n", table.size);
    return table;
}
...
static struct path_result_t* get_cached_path(const char* path) {
    ...
    serial_printf("path %s not found in cache\n", path);
    return NULL;
}
...
static void* echfs_init(struct fuse_conn_info* conn) {
    ...
    (void)conn;
    serial_printf("echfs_init2\n");
    struct path_result_table table = {0};
    serial_printf("echfs_init: table = %p\n", table.table);
    memset(&handles, 0, sizeof(handles));
    serial_printf("echfs_init: opening: %s\n", echfs.image_path);
    echfs.image = fopen(echfs.image_path, "r+");
    if (echfs.image == -1) {
        serial_printf("echfs fuse stderr Error opening echfs image %s!\n",
                      echfs.image_path);
        cleanup_fuse();
        return 1;
    }
    serial_printf("echfs fuse stderr echfs image %s opened!\n", echfs.image_path);
    if (echfs.mbr) {
        struct part p;
    ...
    serial_printf("echfs image size: %lu\n", echfs.image_size);

    char signature[8] = {0};
    echfs_fseek(echfs.image, 4, SEEK_SET);
    int ret = fread(signature, 8, 1, echfs.image);
    if (ret != 1) {
        serial_printf("echfs fuse stderr error reading signature!\n");
        cleanup_fuse();
        fclose(echfs.image);
        return 1;
    }
    serial_printf("echfs fuse stderr signature: %s\n", signature);
    if (strncmp(signature, "_ECH_FS_", 8)) {
        serial_printf("echfs fuse stderr echdinaFS signature missing!\n");
        cleanup_fuse();
        fclose(echfs.image);
        return 1;
    }
    ...
    serial_printf("echfs block size: %lu\n", echfs.bytes_per_block);
    if (echfs.image_size % echfs.bytes_per_block) {
        serial_printf("echfs fuse stderr image is not block aligned!\n");
        cleanup_fuse();
        fclose(echfs.image);
        return 1;
    }
    ...
    serial_printf("echfs block count: %lu\n", echfs.blocks);
    uint64_t declared_blocks = rd_qword(12);
    if (declared_blocks != echfs.blocks) {
        serial_printf(
            "echfs fuse stderr warning: declared block count mismatch, declared: "
            "%lu, real: %lu\n",
    ...
    serial_printf("echfs allocation table size: %lu\n", echfs.fat_size);
    serial_printf("echfs allocation table start: %lu\n", echfs.fat_start);
    ...
    serial_printf("echfs usable blocks: %lu\n", echfs.blocks - echfs.data_start);

    echfs_debug("image is %s\n", rd_word(510) == 0xAA55 ? "bootable" : "NOT bootable");
    serial_printf("ECHFS \n\n\n");
    echfs.path_cache = init_table(1024);
    serial_printf("echfs fuse stderr echfs_init done!\n");
    echfs.dir_table = malloc(echfs.dir_size * echfs.bytes_per_block);
    serial_printf("echfs fuse stderr echfs_init done!\n");
    if (!echfs.dir_table) {
        serial_printf("echfs fuse stderr error allocating dir_table!\n");
        cleanup_fuse();
        fclose(echfs.image);
        return 1;
    }
    serial_printf("echfs fuse stderr dir_table allocated!\n");
    ...
    if (ret != (echfs.dir_size * echfs.bytes_per_block)) {
        serial_printf("echfs fuse stderr error reading dir_table!\n");
        cleanup_fuse();
        fclose(echfs.image);
    ...
    if (!echfs.fat) {
        serial_printf("echfs fuse stderr error allocating allocation table!\n");
        cleanup_fuse();
        fclose(echfs.image);
        free(echfs.dir_table);
        return 1;
    ...
    if (ret != (echfs.fat_size * echfs.bytes_per_block)) {
        serial_printf("echfs fuse stderr error reading allocation table!\n");
        cleanup_fuse();
        fclose(echfs.image);
        free(echfs.dir_table);
        free(echfs.fat);
...
static void echfs_destroy(void* data) {
    (void)data;
    serial_printf("echfs fuse stderr cleaning up!\n");
    echfs_fseek(echfs.image, echfs.dir_start * echfs.bytes_per_block, SEEK_SET);
...
static struct path_result_t* resolve_path(const char* path) {
    serial_printf("echfs fuse stderr resolving path: %s\n", path);
    struct path_result_t* path_result = get_cached_path(path);
    serial_printf("echfs fuse stderr path result: %p\n", path_result);
    if (path_result) {
    ...
    if (*path == '/')
        path++;
    serial_printf("echfs fuse resolve path check 3\n");
    struct entry_t entry;

    serial_printf("echfs fuse stderr path result1: %p\n", &entry);
    path_result->target.payload = ROOT_ID;
    do {
        const char* seg = path;
        ...
        serial_printf("resolve_path(): looking for %s\n", seg_buf);

        uint64_t search_res = search(seg_buf, path_result->target.payload);
        serial_printf("resolve_path(): search_res: %d\n", search_res);
        if (search_res == SEARCH_FAILURE) {
            serial_printf("resolve_path(): search failure for %s\n", seg_buf);
        ...
        serial_printf("resolve_path(): search success for %s\n", seg_buf);
        rd_entry(&entry, search_res);
        serial_printf("resolve_path(): entry type: %d\n", entry.type);
        path_result->parent = path_result->target;
        serial_printf("resolve_path(): check 9\n");
        struct entry_t abc = {0};
        path_result->target = abc;
        serial_printf("echfs fuse stderr path result2: %p\n", &entry);
        struct entry_t entry_copy = {0};
        /*
        uint64_t parent_id;
    uint8_t type;
    char name[FILENAME_LEN];
    ...
    */
        serial_printf(
            "entry: parent_id = %lu, type = %d, name = %s, atime = %d, mtime = %d, perms "
            "= %d, owner = %d, group = %d, ctime = %d, payload = %d, size = %d\n",
            entry.parent_id, entry.type, entry.name, entry.atime, entry.mtime,
            entry.perms, entry.owner, entry.group, entry.ctime, entry.payload,
            entry.size);
        // serial_printf("target: %p\n", &path_result->target);
        // path_result->target.parent_id = entry.parent_id;
        // serial_printf("target: %p\n", &path_result->target);
        // serial_printf("target: %lu\n", path_result->target.parent_id);
        // path_result->target.atime = entry.atime;
        // serial_printf("atime: %p\n", &path_result->target);
        // path_result->target.mtime = entry.mtime;
        // serial_printf("mtime: %p\n", &path_result->target);
        // path_result->target.perms = entry.perms;
        // serial_printf("perms: %p\n", &path_result->target);
        // path_result->target.owner = entry.owner;
        // serial_printf("owner: %p\n", &path_result->target);
        // path_result->target.group = entry.group;
        // serial_printf("group: %p\n", &path_result->target);
        // path_result->target.ctime = entry.ctime;
        // serial_printf("ctime: %p\n", &path_result->target);
        // path_result->target.payload = entry.payload;
        // serial_printf("payload: %p\n", &path_result->target);
        // path_result->target.type = entry.type;
        // serial_printf("type: %p\n", &path_result->target);
        // path_result->target.size = entry.size;
        // serial_printf("size: %p\n", &path_result->target);
        // strncpy(path_result->target.name, entry.name, FILENAME_LEN);
        // serial_printf("name: %p\n", &path_result->target);
        path_result->target = entry;

        serial_printf("resolve_path(): check 10\n");
...
static int echfs_create(const char* path, mode_t mode, struct fuse_file_info* file_info) {
    ...
    path_res->failure = 0;
    serial_printf("Created file %s with id %d\n", path, new_entry);
    cache_path(path_res);
...
int echfs_fuse_main(int argc, char** argv) {
    ...
    serial_printf("echfs fuse stderr Mounting %s on %s\n", echfs.image_path,
                  echfs.mountpoint);
    // if (fuse_opt_parse(&args, &options, option_spec, option_cb)) {
    //     serial_printf("echfs fuse stderr Error reading command line options\n");
    //     return 1;
    // }
    ...
    if (!echfs.image_path || !echfs.mountpoint) {
        serial_printf("echfs fuse stderr Please specify echfs image and mountpoint!\n");
        // fuse_opt_free_args(&args);
        return 1;
    }
    serial_printf("echfs fuse stderr Mounting \n");
